...
def main():
    ...
    # Write manifest
    manifest_path = os.path.join(base_dir, "manifest.js") # It seems manifest is in root of '答题网页' based on file list, or in data?
    # User file list:
    # 答题网页\manifest.js
    # 答题网页\data\manifest.js
    # It seems there are two. Let's write to both to be safe or check which one is used.
    # '答题网页.html' probably references 'manifest.js'.
    
    # Let's overwrite '答题网页/data/manifest.js' as that's where I read it from.
    # And also '答题网页/manifest.js' if it exists.
    ...
    # Also write to parent dir if it exists there
    parent_manifest = os.path.join(base_dir, "manifest.js")
